...
求解的目标表达式为：
y = 10 * math.sin(5 * x) + 7 * math.cos(4 * x) 最大值
"""
...
def main():
    ...
    #plot_obj_func()
    pop_size = 500  # 种群数量
    upper_limit = 10  # 基因中允许出现的最大值，定义域的最大值【-2,2】
    chromosome_length = 10  # 染色体长度16位长度对于优化bp神经网络初始值
    iter = 500#迭代次数
    pc = 0.6 # 杂交概率
    pm = 0.01  # 变异概率
...
# 解码并计算值
def decode_chromosome(pop, chromosome_length, upper_limit):#500,10,10
    X = []
    for ele in pop:
        temp = 0
...
def calc_obj_value(pop, chromosome_length, upper_limit):#500个个体，每个个体只有一条染色体，每条染色体有10位。
    obj_value = []
